...
import org.Lee.service.ReplyService2;
...
public class ReplyController2 {
	...
	public ResponseEntity<String> create(@RequestBody ReplyVO2 vo){
		...
		log.info("ReplyVO2: " + vo);
		int insertCount = service.register(vo);
		
		log.info("Reply2 INSERT COUNT: " + insertCount);
	...
	@GetMapping(value="/{rno}",
			produces = {
					MediaType.APPLICATION_XML_VALUE,
					MediaType.APPLICATION_JSON_UTF8_VALUE})
	public ResponseEntity<ReplyVO2> get(@PathVariable("rno") int rno){
		log.info("get2 : " + rno);
	...
	@GetMapping(value="/pages/{bno}/{page}",
			produces = {
					MediaType.APPLICATION_XML_VALUE,
					MediaType.APPLICATION_JSON_UTF8_VALUE})
	public ResponseEntity<ReplyPageDTO2> getList(
			@PathVariable("page") int page, @PathVariable("bno") int bno){
		Criteria cri = new Criteria(page,10);
		log.info("get2 Reply List bno : " + bno);
		log.info("cri2 : " + cri);
